...
/// 执行 Maven 完整安装流程：冲突清理 → 下载 → ZIP 解压 → settings.xml → 环境变量。
pub async fn install(
    ...
    // 安装前清理旧版本冲突：移除旧 PATH 条目、重置 MAVEN_HOME 和 M2_HOME
    crate::install::conflict::resolve_conflicts(app, "maven")?;

    emit_status(
        app,
        "maven",
        "download",
        &format!("正在下载 Maven {version}..."),
    );
...
<settings xmlns="http://maven.apache.org/SETTINGS/1.2.0"
    ...
    std::fs::write(&path, settings.as_bytes()).map_err(|e| format!("写入 settings.xml 失败: {e}"))
}
