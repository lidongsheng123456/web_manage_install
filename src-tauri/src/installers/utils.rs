...
fn find_first_dir(dir: &str) -> Result<String, String> {
...
pub fn extract_and_move(
    ...
    let source = format!("{extract_dir}\\{top_dir}");
    // 如果解压出的顶层目录不存在，尝试查找第一个目录
    let source = if Path::new(&source).is_dir() {
        source
    } else {
        find_first_dir(&extract_dir)?
    };
