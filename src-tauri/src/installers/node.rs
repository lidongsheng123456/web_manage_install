...
pub async fn install(
    ...
    emit_status(app, "nodejs", "download", &format!("正在下载 Node.js v{version}..."));
    let msi_path = download::download_with_version("nodejs", version, temp_dir, on_progress).await?;
    ...
    let output = Command::new("msiexec")
        .args(["/i", &msi_path, "/qn", "/norestart",
               &format!("INSTALLDIR={node_dir}"), "ADDLOCAL=ALL"])
        .output()
    ...
    if exit_code != 0 && exit_code != 3010 {
        ...
        return Err(format!("Node.js MSI 安装失败 (exit {}): {stderr}", exit_code));
    }
