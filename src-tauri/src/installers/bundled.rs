...
pub async fn download_idea(
    ...
    emit_done(app, "idea", true, &format!("IDEA 安装包已下载到 {dest}，请手动双击安装"));
    Ok(())
}
...
pub async fn download_navicat(
    ...
    let src = download::download_with_version("navicat", "16.2", temp_dir, on_progress).await?;
    ...
    emit_done(app, "navicat", true, &format!("Navicat 安装包已下载到 {dest}，请手动双击安装"));
    Ok(())
}
...
pub async fn download_redis(
    ...
    let zip_path = download::download_with_version("redis", "5.0.14.1", temp_dir, on_progress).await?;

    emit_status(app, "redis", "install", "正在解压 Redis...");
    let target = crate::installers::utils::extract_and_move(&zip_path, install_root, "redis", "redis")?;
    ...
    emit_done(app, "redis", true, &format!("Redis 已解压到 {target}（绿色免安装，双击 redis-server.exe 启动）"));
    Ok(())
}
